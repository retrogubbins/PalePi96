...
//              Z80_ICount=240;
        ...
        do
        {
                ...
                ++R.R;
        //      cyc++; //used for the SOUND stuff
                //              port84cyc=(z80_icount_last-Z80_ICount);
                //tapecycles+=(z80_icount_last-Z80_ICount);
                //z80_icount_last=Z80_ICount;
...
//              if (curpc==0x0000)
//                                      reset_puter();          //This is a trap for user doing CALL0  - i reset the disks etc.


//              if (curpc==trapaddr%65536)
//              if (curpc==0xe000)
//                      trapc=1;
//              if(trapc==1)
                        tracedump(curpc);
...
//              if(trace_on)
//              {
//                      Z80_RegisterDump_String ((unsigned char *)lbl2,(unsigned char *)lbl3);
//                      fprintf(stream2,lbl2);
//              }
/*
                if(trapthis && trapped) 
                {
                        ...
                        sprintf(lbl1,"PC is %04x  Opcode %02x  Trapped %d times\n\n",trapaddr,opcode,trapc);
                        Z80_RegisterDump_String ((unsigned char *)lbl2,(unsigned char *)lbl3);
                        strcat(lbl1,lbl2);
                        strcat(lbl1,lbl3);
//                      ret=MessageBox(NULL,lbl1,"PALE Debug - Press Yes to Exit, CANCEL to NoTrap",MB_YESNOCANCEL | MB_DEFBUTTON2);
//                      if(ret==IDYES){exit(1);}
                        if(ret==IDNO)
                        {
//                                      int tmp;
...
*/
                ...
                Z80_ICount-=num_cycs;
//              tapecycles+=num_cycs;
//      no      if(tapecycles>128)//good output
//      no      if(tapecycles>40)//got Filename at 0
//      no      if(tapecycles>tape_spd+tape_spd_adjust)
//      no              update_tape();//                Periodic ?22050 Hz (original time) update
...
//              {
//              if(sound_running==1)update_sound();
//              }
                
        //      if(blank_mono==1)
        //      {
        //              blank_mono=0;
                        //Z80_ICount=0;//ONLY works in Accurate vid mode
        //      }
        }
        while (Z80_ICount>0);
...
//      if(enable_soundstream==1)// && sound_cyc++>3)
//      {
//              sound_cyc=0;
//              play_sound();//plays current sound buffer, if anything
//      }
        return Z80_Running;
}
